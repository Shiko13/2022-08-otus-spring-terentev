window.onload = function () {
    ...
    function onBookFormSubmit() {
        ...
        let authorId = document.getElementById("book-author-select")
            .querySelector("option:checked").value;
        let genreId = document.getElementById("book-genre-select")
            .querySelector("option:checked").value;
    ...
    function onCommentDelete(btn) {
        if (confirm('Are you sure you want to delete this record?')) {
            let row = btn.parentElement.parentElement;
            let commentId = row.cells[0].innerHTML;
            let bookId = document.getElementById('book-id-for-comments-input'.value);
            ...
            .then((response) => {
                response.json().then(
                    comments => {
                        ...
                        let newTBody = document.createElement('tbody');
                        fillComments(comments, newTBody);
                        let oldTBody = document.getElementById('tab-comments-data')
                            .getElementsByTagName('tbody')[0];
                        oldTBody.parentNode.replaceChild(newTBody, oldTBody);
    ...
    function fillComments(comments, newTBody) {
